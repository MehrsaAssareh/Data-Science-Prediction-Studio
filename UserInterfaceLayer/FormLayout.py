from ttkbootstrap import Button, Combobox, Entry, Label

READONLY_VALUE_FOREGROUND = '#1E9E93'
READONLY_VALUE_FONT = ('Arial', 10, 'bold')


def apply_form_field_layout(form_frame, entry_width=24, combo_width=22):
    for child in form_frame.winfo_children():
        ...
        if isinstance(child, Label):
            ...
            if column in (0, 2):
                child.grid_configure(padx=(10, 2), pady=4, sticky='w')
            continue

        if isinstance(child, Combobox):
            child.configure(width=combo_width)
            child.grid_configure(padx=(2, 10), pady=4, sticky='ew')
            continue
        ...
        apply_form_field_layout(child, entry_width, combo_width)
